...
const checkboxVariants = cva('peer cursor-pointer rounded border-2 border-slate-400 transition-colors duration-300 ease-in-out ' +
...
export const Checkbox = forwardRef(({ size = 'medium', children, className, label, indeterminate, ...rest }, ref) => {
    const id = useId();
    ...
    return (<div className={clsxMerge(checkboxContainerVariants({ size }), className)}>
        <input id={rest.id ?? id} ref={innerRef} type='checkbox' aria-checked={indeterminate ? 'mixed' : checked} onChange={handleChange} className={clsxMerge(checkboxVariants({ size }), className)} checked={checked} {...rest}/>
        {label && (<Label htmlFor={rest.id ?? id} size='small' className='whitespace-nowrap peer-disabled:cursor-not-allowed peer-disabled:text-slate-400 hover:cursor-pointer'>
            {label}
          </Label>)}
      </div>);
});
