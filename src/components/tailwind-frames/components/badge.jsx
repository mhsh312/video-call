...
const badgeVariants = cva('inline-flex rounded-[64px] border text-center font-semibold transition-all duration-300 ease-in-out', {
    variants: {
        dotOnly: {
            true: 'rounded-full',
            false: '',
...
const Root = ({ variant = 'default', size = 'medium', dotOnly = false, className, label, children, ...rest }) => (<span className={clsxMerge(badgeVariants({ variant, label: Boolean(label), size, dotOnly }), className)} {...rest}>
...
export function Badge(props) {
    ...
    return (<div className='inline-flex items-center justify-start'>
      <Root {...props}/>
